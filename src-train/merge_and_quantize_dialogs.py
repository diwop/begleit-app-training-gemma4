#!/usr/bin/env python3
"""
Merges a trained LoRA adapter from the dialogs flow into the base bfloat16 Gemma 4 model and compresses
...
def merge_via_axolotl(
    config_path: str, adapter_dir: str, output_dir: str
) -> bool:
    """Try merging using Axolotl's native CLI merge_lora for Gemma 4."""
    print("[INFO] Attempting merge via Axolotl CLI (axolotl.cli.merge_lora)...")
    env = os.environ.copy()
    ...
    try:
        ...
        print(
            f"[WARNING] Axolotl merge failed or unavailable: {e}. Falling back to PEFT merge.",
            file=sys.stderr,
        )
    return False
...
def merge_via_peft(
    base_model_snapshot: str, adapter_path: Path, merged_bf16_path: Path
) -> None:
    ...
    print(
        f"[STEP 1/3] Saving merged bfloat16 checkpoint to: {merged_bf16_path}"
    )
    merged_model.save_pretrained(str(merged_bf16_path))
    tokenizer = AutoTokenizer.from_pretrained(
        base_model_snapshot, trust_remote_code=True, local_files_only=True
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Merge dialogs LoRA adapter into Gemma 4 base model and compress to FP8"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="src-train/config_dialogs.yml",
        help="Path to training config_dialogs.yml",
    )
    parser.add_argument(
        "--base-model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to temporary merged BF16 model (defaults to $SLURM_TMPDIR or ./local/merged/gemma-4-26b-a4b-it-bf16_dialogs)",
    )
    parser.add_argument(
        "--output-fp8-dir",
        type=str,
    ...
    if not adapter_path.exists():
        print(
            f"[ERROR] Adapter directory not found at: {adapter_path}",
            file=sys.stderr,
        )
        print(
            "[INFO] Please complete adapter fine-tuning before running merge & quantization.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if args.merged_bf16_dir:
        ...
        if slurm_tmp and Path(slurm_tmp).is_dir():
            merged_bf16_path = Path(slurm_tmp) / "merged-gemma4-dialogs-bf16"
        else:
            merged_bf16_path = Path(
                "./local/merged/gemma-4-26b-a4b-it-bf16_dialogs"
            )
    merged_bf16_path.parent.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("      Gemma 4 Dialogs Adapter Merge & FP8 Quantization")
    print("=" * 60)
    ...
    merged_ok = merge_via_axolotl(
        args.config, str(adapter_path), str(merged_bf16_path)
    )
    if not merged_ok:
    ...
    try:
        ...
        print(
            f"[ERROR] Required package for FP8 compression missing: {e}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    processor = AutoProcessor.from_pretrained(
        str(merged_bf16_path), trust_remote_code=True
    )
    ...
    print("\n" + "=" * 60)
    print(
        f"[SUCCESS] Merged & Compressed FP8 dialogs model is ready at: {output_fp8_path}"
    )
    print("=" * 60)
