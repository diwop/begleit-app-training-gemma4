...
Containers for Axolotl (training) and sglang (inference) are run using apptainer on the HSUper GPU nodes.
...
The pipeline is strongly based on the heavy dependencies Axolotl and sglang being launched by rather simple Python scripts that change fast.
...
Axolotl and sglang are hardly changed while the scripts for training and inference are subject to faster changes.
...
The training code is run in an Axolotl container while the code for inference is run in an sglang container.