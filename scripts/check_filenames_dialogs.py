#!/usr/bin/env python3
"""
Script to validate and auto-fix filename patterns in data/raw_dialogs.
- Deletes .docx files
...
def fix_and_check_filenames(directory: Path) -> list[tuple[str, str]]:
    ...
    print(
        "[INFO] Applying automatic fixes (unifying 'Leichte_Sprache', deleting"
        " .docx files, fixing 3-digit prefixes)..."
    )
    deleted_count = 0
    renamed_count = 0
    ...
    for path in sorted(directory.iterdir()):
        ...
        if (
            ...
            or filename.startswith(IGNORE_PREFIXES)
            or filename.endswith(IGNORE_SUFFIXES)
        ):
            continue
        ...
        if new_path != path:
            if new_path.exists():
                print(
                    f"  [DELETED DUPLICATE] '{filename}' (identical file"
    ...
    print(
        f"[INFO] Auto-fix complete: Deleted {deleted_count} file(s), Renamed"
        f" {renamed_count} file(s).\n"
    )
...
def main() -> None:
    dir_path = (
        Path(sys.argv[1]) if len(sys.argv) > 1 else Path("data/raw_dialogs")
    )
    ...
    if not mismatches:
        print(
            " [SUCCESS] All filenames in data/raw_dialogs match the expected"
            " pattern!"
        )
    else:
        print(
            f" [WARNING] Found {len(mismatches)} remaining mismatched"
            " file(s):\n"
        )
