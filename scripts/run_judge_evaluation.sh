...
# Tensor Parallel size (defaults to 4 for L40S cluster nodes)
export TENSOR_PARALLEL_SIZE="${TENSOR_PARALLEL_SIZE:-4}"
export JUDGE_MODEL_NAME="${JUDGE_MODEL_NAME:-nvidia/Llama-4-Scout-17B-16E-Instruct-FP8}"
export MAX_EVAL_SAMPLES="${MAX_EVAL_SAMPLES:-0}"

echo "============================================================"
echo " Starting LLM-as-a-Judge Evaluation (Llama 4 Scout FP8)"
echo "============================================================"
...
echo "[INFO] Judge Model     : ${JUDGE_MODEL_NAME}"
echo "[INFO] TP Size         : ${TENSOR_PARALLEL_SIZE}"
echo "[INFO] Max Samples     : ${MAX_EVAL_SAMPLES}"
echo "============================================================"
...
if [ -n "${SLURM_TMPDIR:-}" ] && [ -d "${SLURM_TMPDIR}" ]; then
  SCRATCH_ROOT="${SLURM_TMPDIR}/${SLURM_JOB_ID:-judge_scratch}"
...
# Execute judge_evaluation.py inside SGLang container in offline mode across 4 GPUs
apptainer exec \
  --nv \
  ...
  --env JUDGE_MODEL_NAME="${JUDGE_MODEL_NAME}" \
  --env MAX_EVAL_SAMPLES="${MAX_EVAL_SAMPLES}" \
  ...
  "${CONTAINER_PYTHON}" src-eval/judge_evaluation.py

echo "============================================================"
echo "[SUCCESS] LLM-as-a-Judge Evaluation finished!"
echo "============================================================"
